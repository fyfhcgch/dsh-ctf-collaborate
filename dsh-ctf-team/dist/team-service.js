...
const CATEGORIES = new Set(['pwn', 'crypto', 'web', 'rev', 'misc', 'forensic']);
const STATUSES = new Set(['pending', 'solving', 'solved']);
const MAX_TITLE = 200;
...
export class TeamService {
    ...
    operationSink;
    localPeerId;
    constructor(db, broadcast, agentRunner, operationSink, localPeerId = 'local') {
        this.db = db;
    ...
    listChallenges() { return this.db.listChallenges(); }
    getDetail(challengeId) {
    ...
    createChallenge(input) {
        ...
        if (this.db.getChallenge(challenge.challengeId))
            throw new TeamInputError('challengeId already exists', 'conflict');
    ...
    updateChallenge(challengeId, input) {
        ...
        if (Object.keys(patch).length === 0)
            throw new TeamInputError('At least one challenge field is required');
    ...
    async spawnAgent(challengeId, ownerUserId, prompt) {
        const id = this.requireChallenge(challengeId);
        const runner = this.agentRunner;
        if (!runner || !runner.available)
            throw new TeamInputError('Agent tasks are not configured', 'unsupported');
    ...
    applyRemoteOperation(operation) {
        ...
        switch (operation.kind) {
            case 'challenge_upsert': {
                ...
                if (current)
                    this.db.updateChallenge(challenge);
                else
                    this.db.insertChallenge(challenge);
            ...
            case 'thought_add': {
                ...
                if (!this.db.getChallenge(thought.challengeId))
                    return 'pending';
                this.db.insertThought(thought);
            ...
            case 'evidence_add': {
                ...
                if (!this.db.getChallenge(evidence.challengeId))
                    return 'pending';
                this.db.insertEvidence(evidence);
            ...
            case 'task_upsert': {
                ...
                if (!this.db.getChallenge(task.challengeId))
                    return 'pending';
    ...
    requireChallenge(value) {
        const id = requireId(value, 'challengeId');
        if (!this.db.getChallenge(id))
            throw new TeamNotFoundError('Challenge not found');
        return id;
    }
}
function requireId(value, field) {
    ...
    if (id.includes('/') || id.includes('\\'))
        throw new TeamInputError(`${field} contains an invalid path separator`);
    return id;
}
...
function requiredText(value, field, maxLength) {
    if (typeof value !== 'string')
        throw new TeamInputError(`${field} must be a string`);
    const text = value.trim();
    if (!text)
        throw new TeamInputError(`${field} is required`);
    if (text.length > maxLength)
        throw new TeamInputError(`${field} exceeds ${maxLength} characters`);
    return text;
}
...
function boundedActor(value, field) {
    if (value === undefined || value === null || value === '')
        return 'web-user';
    return requiredText(value, field, 128);
}
function category(value, defaultValue = true) {
    if (value === undefined || value === null || value === '') {
        if (defaultValue)
            return 'misc';
...
function status(value, defaultValue = true) {
    if (value === undefined || value === null || value === '') {
        if (defaultValue)
            return 'pending';
...
function attachmentPaths(value) {
    ...
    if (!Array.isArray(value) || value.length > 64)
        throw new TeamInputError('attachmentPaths must contain at most 64 strings');
    return value.map((item, index) => requiredText(item, `attachmentPaths[${index}]`, 4096));
}
function evidenceType(value) {
    if (value === undefined || value === null || value === '')
        return 'log';
    if (value === 'tool_output' || value === 'file_extract' || value === 'log')
        return value;
    throw new TeamInputError('type is invalid');
}
function compareVersion(operation, version) {
    return operation.createdAt - version.createdAt || operation.opId.localeCompare(version.opId);
}
function remoteText(value, field, max = MAX_CONTENT) {
    if (typeof value !== 'string' || !value.trim() || value.length > max)
        throw new TeamInputError(`${field} is invalid`);
    return value;
}
function parseRemoteChallenge(value) {
    ...
    if (!Array.isArray(attachments) || attachments.some((item) => typeof item !== 'string'))
        throw new TeamInputError('attachmentPaths is invalid');
    return {
...
function parseRemoteThought(value) {
...
function parseRemoteEvidence(value) {
    const type = value.type;
    if (type !== 'tool_output' && type !== 'file_extract' && type !== 'log')
        throw new TeamInputError('type is invalid');
...
function parseRemoteTask(value) {

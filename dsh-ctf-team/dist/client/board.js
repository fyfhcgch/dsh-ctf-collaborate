...
const statuses = ['pending', 'solving', 'solved'];
const evidenceTypes = ['tool_output', 'file_extract', 'log'];
...
export class TeamBoard {
    ...
    async refresh(options = {}) {
        ...
        try {
            ...
            if (!this.state.selectedId && challenges[0])
    ...
    onInput(event) {
        ...
        if (!field?.name)
            return;
        const form = field.closest('form[data-action]');
    ...
    async onSubmit(event) {
        ...
        if (action === 'evidence') {
            await this.run('证据已添加', async () => {
                await this.remote.addEvidence({ challengeId, type: (text(data, 'type') || 'log'), content: text(data, 'content') });
    ...
    render() {
        ...
        if (!shell)
            return;
        if (this.root)
            this.root.setAttribute('data-sidebar-wide', this.sidebarWide ? '1' : '0');
    ...
    renderChallengeList() {
        if (!this.state.challenges.length)
            return '<div class="ctf-empty small">暂无挑战</div>';
    ...
    renderDetail(detail) {
      ...
      <nav class="ctf-tabs">
        ${['overview', 'notes', 'evidence', 'thoughts', 'tasks', 'p2p'].map((item) => `<button type="button" data-action="tab" data-tab="${item}" class="${tab === item ? 'active' : ''}">${tabLabel(item)}</button>`).join('')}
      </nav>

...
export function createDb(dbPath: string): TeamDb {
  ...
  const named = (value: object) => value as Record<string, any>
  const ensureOpen = () => { if (closed) throw new Error('Team database is closed') }
  ...
  return {
    ...
    appendOperation: (operation: TeamOperation) => {
      ...
      const row = db.prepare('SELECT sequence FROM team_operations WHERE opId=?').get(operation.opId) as { sequence?: number | bigint } | undefined
    ...
    listOperations: (afterSequence, limit): SyncBatch => {
      ensureOpen()
    ...
    getVersion: (scope, entityId): TeamVersion | null => {
      ensureOpen()
      const row = db.prepare('SELECT createdAt,opId FROM team_versions WHERE scope=? AND entityId=?').get(scope, entityId) as { createdAt?: number; opId?: string } | undefined

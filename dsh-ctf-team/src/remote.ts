...
const sharedNote = z.object({ challengeId: z.string(), content: z.string(), updatedBy: z.string(), updatedAt: z.number() })
...
const task = z.object({ taskId: z.string(), challengeId: z.string(), ownerUserId: z.string(), expertType: z.union([z.literal('general'), z.literal('pwn'), z.literal('reverse')]), prompt: z.string(), done: z.boolean(), result: z.string(), createdAt: z.number() })
const detail = z.object({ challenge, sharedNote: sharedNote.nullable(), notes: z.array(note), thoughts: z.array(thought), evidence: z.array(evidence), tasks: z.array(task) })
...
const spawnInput = z.object({ challengeId: z.string(), ownerUserId: z.string().optional(), expertType: z.union([z.literal('general'), z.literal('pwn'), z.literal('reverse')]).optional(), prompt: z.string() })
...
const operation = z.object({ sequence: z.number(), opId: z.string(), peerId: z.string(), kind: z.union([z.literal('challenge_upsert'), z.literal('challenge_delete'), z.literal('note_add'), z.literal('shared_note_upsert'), z.literal('thought_add'), z.literal('evidence_add'), z.literal('task_upsert')]), payload: z.unknown(), createdAt: z.number() })
...
function strict(typeSymbol: string, schema: { parse(value: unknown): unknown }): TypertCodec {
  return { mode: 'strict', typeSymbol, schema }
}

function descriptor(method: string, parameters: readonly { name: string; wire: string; source: 'json'; codec: TypertCodec; acceptsUndefined?: true }[], result: TypertCodec) {
  return { id: `dsh-ctf-team#ctfTeam/${method}`, service: 'ctfTeam', namespace: 'ctfTeam', method, invocation: { kind: 'direct' as const }, parameters, result }
}
